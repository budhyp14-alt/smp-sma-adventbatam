...
export default function EditorialKepsekIndex() {
  const allArticles = editorialsData?.kepsek || [];
      ...
      <section className="w-full flex-1 pb-16 px-4 sm:px-8">
        <div className="max-w-7xl mx-auto space-y-8">
          {displayedArticles.map((item, idx) => (
            <div 
              key={idx} 
              className="bg-white p-6 rounded-xl shadow-sm border-t-4 border-[#8B0000] flex flex-col md:flex-row gap-6 items-start group"
            >
              {/* 1. FOTO THUMBNAIL */}
              <Link 
                href={`/editorial/detail?id=${item.id}`} 
          ...
          {visibleCount < allArticles.length && (
            <div className="pt-6 flex justify-center">
              <button
                ...
                className="bg-[#8B0000] hover:bg-red-800 text-white font-bold text-sm py-3 px-8 rounded-full shadow-md transition-transform transform hover:scale-105 flex items-center gap-2 cursor-pointer"
              >
