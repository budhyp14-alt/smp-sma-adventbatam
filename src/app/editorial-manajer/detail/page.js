...
export const metadata = {
  title: "Operations Manager Editorial Detail - SMP SMA Advent Batam",
};

export default function EditorialManajerDetail({ searchParams }) {
  const itemId = searchParams?.id || editorialsData.manajer[0].id;
  const article = editorialsData.manajer.find((a) => a.id === itemId) || editorialsData.manajer[0];
    ...
    <main className="min-h-screen bg-[#E5DCC3] font-sans flex flex-col">
      <section className="w-full pt-8 pb-4 px-4 sm:px-8 max-w-5xl mx-auto shrink-0">
        <div className="text-xs text-slate-700 font-semibold mb-6 flex items-center gap-1 flex-wrap">
          ...
          <span>-</span>
          <Link href="/editorial-manajer" className="text-[#047857] hover:underline">Operations Manager</Link>
          <span>-</span>
          <span className="text-slate-600">Detail</span>
        </div>
        <article className="bg-white p-6 sm:p-10 rounded-2xl shadow-md border-t-8 border-[#047857]">
          <span className="bg-[#047857] text-white text-xs font-bold px-3 py-1 rounded">OPERATIONS EDITORIAL</span>
          ...
          <div className="mt-8 pt-6 border-t border-slate-200 flex justify-between items-center">
            <Link href="/editorial-manajer" className="bg-[#8B0000] hover:bg-red-800 text-white font-bold text-xs py-2.5 px-6 rounded transition-colors">
