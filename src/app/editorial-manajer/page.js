      ...
      <section className="w-full flex-1 pb-16 px-4 sm:px-8">
        <div className="max-w-7xl mx-auto space-y-8">
          {articles.map((item, idx) => (
            <div key={idx} className="bg-white p-6 rounded-xl shadow-sm border-t-4 border-[#047857] flex flex-col md:flex-row gap-6 items-start">
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div>
                  <Link href={`/editorial-manajer/detail?id=${item.id}`} className="bg-[#8B0000] hover:bg-red-800 text-white font-bold text-xs py-2 px-5 rounded shadow-sm inline-block transition-colors">
                    Read More →
                  </Link>
                </div>
              </div>
